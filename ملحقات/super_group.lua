...
sendPhoto(msg.to.id,msg.id_,0,1,nil,data.photos_[0].sizes_[1].photo_.persistent_id_,'┐ ايديك ↞'..msg.from.id..'🎫؛\n┤ معرفـك ↞ '..userxn..'Ⓜ️؛\n┤ رتبتـك ↞'..get_rank(msg)..'؛\n┤ رسائلك ↞ {'..msgs..'}📨؛\n┤ تفاعلك  ↞ '..get_ttl(msgs)..'؛\n┤ جهاتك ↞ {'..(tonumber(redis:get(boss..':addusers_group:'..msg.to.id..':'..msg.from.id)) or 0)..'}📮؛\n┤ الوقت  ↞ '..os.date("%I:%M %p")..'⏱؛',dl_cb,nil) else
sendMsg(msg.to.id,msg.id_,'🚸*¦* لا يوجد صوره في بروفايلك ...!\n\n👤*¦* اسمك » ['..check_name(namecut(msg.from.first_name))..']\n🎫*¦* معرفك » ['..userxn..']\n🏷*¦* ايديك » (*'..msg.from.id..'*)\n📮*¦* رتبتك » '..get_rank(msg)..'\n⭐️*¦* تفاعلك » '..get_ttl(msgs)..'\n📨*¦* رسائلك » (*'..msgs..'*) رساله\n➖','md')
end end,nil) else
return 'الايدي معطل عزيزي'
end end
...
if matches[1] == "الغاء التقييد" and is_mod(msg) then
...
if matches[1] == "ارفع عضو مميز" and is_mod(msg) then
...
if matches[1] == "انزل عضو مميز" and is_mod(msg) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "ارفع مدير" and (is_sudo(msg) or is_monsha(msg)) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "انزل مدير" and (is_sudo(msg) or is_monsha(msg)) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "ارفع منشى" and is_sudo(msg) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "انزل منشى" and is_sudo(msg) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "ارفع اداري" and is_owner(msg) then
if not matches[2] and msg.reply_id then
...
if matches[1] == "انزل اداري" and is_owner(msg) then
if not matches[2] and msg.reply_id then
...
if matches[2] == "الماركدون" then return lock_markdown(msg) end
...
if matches[2] == "الماركدون" then return unlock_markdown(msg) end
...
if not redis:get(boss..'rulse:msg'..msg.to.id) then return "👋🏻¦ مرحبا عزيزي  القوانين كلاتي 👨‍💻\n❌¦ أحترم تحترم\n❌¦ ممنوع الدخول خاص \n❌¦ ممنوع مضايقة الاعضاء \n❌¦ ممنوع سرقت معرفات الاعضاء\n❌¦ ممنوع نشر الروابط  \n❌¦ ممنوع  اعاده توجيه\n❌¦ ممنوع التكلم بلطائفه ❌¦ الرجاء احترام المدراء والادمنيه 👨‍⚖"
...
if matches[1] == "كشف الادارين" and not matches[2] and is_owner(msg) then
...
return {patterns ={"^(تقييد) (%d+)$","^(تقييد) (@[%a%d%_]+)$","^(فك التقييد) (%d+)$","^(فك التقييد) (@[%a%d%_]+)$","^(ضع شرط التفعيل) (%d+)$","^(التفاعل) (@[%a%d%_]+)$","^(ايدي) (@[%a%d%_]+)$","^(كشف) (%d+)$","^(كشف) (@[%a%d%_]+)$",'^(ارفع عضو مميز) (@[%a%d%_]+)$','^(ارفع عضو مميز) (%d+)$','^(انزل عضو مميز) (@[%a%d%_]+)$','^(انزل عضو مميز) (%d+)$','^(ارفع اداري) (@[%a%d%_]+)$','^(ارفع اداري) (%d+)$','^(انزل اداري) (@[%a%d%_]+)$','^(انزل اداري) (%d+)$','^(رفع المدير) (@[%a%d%_]+)$','^(ارفع مدير) (%d+)$','^(ارفع منشى) (%d+)$','^(انزل منشى) (%d+)$','^(ارفع منشى) (@[%a%d%_]+)$','^(انزل منشى) (@[%a%d%_]+)$','^(انزل مدير) (@[%a%d%_]+)$','^(انزل مدير) (%d+)$','^(قفل) (.*)$','^(فتح) (.*)$','^(تفعيل) (.*)$','^(تعطيل) (.*)$','^(ضع تكرار) (%d+)$',"^(مسح) (.*)$",'^(منع) (.*)$','^(الغاء منع) (.*)$',"^(حظر عام) (@[%a%d%_]+)$","^(حظر عام) (%d+)$","^(الغاء العام) (@[%a%d%_]+)$","^(الغاء العام) (%d+)$","^(حظر) (@[%a%d%_]+)$","^(حظر) (%d+)$","^(الغاء الحظر) (@[%a%d%_]+)$","^(الغاء الحظر) (%d+)$","^(طرد) (@[%a%d%_]+)$","^(طرد) (%d+)$","^(كتم) (@[%a%d%_]+)$","^(كتم) (%d+)$","^(الغاء الكتم) (@[%a%d%_]+)$","^(الغاء الكتم) (%d+)$","^(رفع مطور) (.*)$","^(تنزيل مطور) (.*)$","^(تعطيل) (-%d+)$","^(الاشتراك) ([123])$", "^(شحن) (%d+)$", "^(git) (.*)$", "(.*)" },run=xboss}